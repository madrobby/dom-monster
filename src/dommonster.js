...
JR.tip("You are using the Ext JS v"+Ext.version+".","There's a newer version available, which potentially includes performance updates.");
  ...
  JR.nodesTips = function(){
    ...
    var nodes = document.getElementsByTagName('*'), i = nodes.length, nodecount = 0, ids = {}, multiIds = [],
    ...
    if(multiIds.length > 0)
    	JR.tip('There '+((multiIds.length==1)?'is ':'are ')+multiIds.length+' multiple id'+((multiIds.length>1)?'s':'')+' for nodes in your document.', 'Node ids must be unique within the HTML document.')
    if(whitespace)
